...
public class GemGame 
{
	...
	{
		System.out.println("Do you want to play human or computer: (h or c)");
		Scanner sc = new Scanner(System.in);
		String answer = sc.nextLine();
		
		boolean isComputer = (answer.equals("c")?  true : false);
		...
		GemList<Gem> player1 = new GemList<Gem>();
		GemList<Gem> player2 = new GemList<Gem>();
		...
		while ((player1.size() < MAX_GEMS) && (player2.size() < MAX_GEMS))
		{
			...
			if(isComputer && turn1 == false)
			{
				//computer
				computer.makeDecision(player1, player2, current);
				
				if(computer.row == 1)
					player1.insertBefore(current, computer.index);
				else
					player2.insertBefore(current, computer.index);
				current = null;
				turn1 = !turn1;
			}
			else
